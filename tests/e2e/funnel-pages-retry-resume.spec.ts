...
test('supports extraction retry with backoff feedback and review transition', async ({ page }) => {
  ...
  await expect(page.getByText('Model E2E')).toBeVisible();
...
test('resumes from artifact checkpoints and exposes recovery actions', async ({ page }) => {
  await setupFunnelBaseMocks(page);
  ...
  await expect(page.getByRole('button', { name: 'Rigenera funnel' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Riprova estrazione' })).toBeVisible();
